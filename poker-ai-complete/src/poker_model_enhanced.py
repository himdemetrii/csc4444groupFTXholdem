...
class EnhancedPokerNet(nn.Module):
    ...
    def forward(self, hole_cards, board_cards, context=None):
        if context is None:
            batch = hole_cards.shape[0]
            context = torch.zeros(batch, 7, device=hole_cards.device)
...
def calculate_hand_strength(hole_cards, board_cards):
    ...
    if hole_ranks[0] == hole_ranks[1]:
        ...
        # If the board adds copies of our pair rank, upgrade strength
        extra_same_rank = sum(1 for r in board_ranks if r == pair_rank)
        total_of_rank = 2 + extra_same_rank  # 2 in our hand + on board
        if total_of_rank == 3:
            # Flopped/turned a set: strong made hand
            strength = max(strength, 0.70)
        elif total_of_rank >= 4:
            # Quads or better: extremely strong
